...
static void RecordCallback(sherpa_onnx::Alsa *alsa) {
  ...
  while (!stop) {
    ...
    std::vector<float> data(samples.size());
    std::transform(samples.begin(), samples.end(), data.begin(),
                   [](float x) { return x * 32768; });

    std::lock_guard<std::mutex> lock(mutex);
    samples_queue.emplace(std::move(data));
    condition_variable.notify_one();
  }
}
...
as the device_name.
  ...
  auto _sence_voice = std::make_unique<SenseVoice>(asr_onnx, tokens);
  auto _vad = std::make_unique<SileroVAD>(vad_onnx);
  ...
  while (!stop) {
    ...
    for (; offset + window_size < buffer.size(); offset += window_size) {
      ...
      std::vector<float> data(window_size);
      std::transform(buffer.data() + offset,
                     buffer.data() + offset + window_size, data.begin(),
                     [](float x) { return x / 32768; });
      auto vad_pred = _vad->predict(data);
      if (!speech_started && vad_pred == "start") {
        speech_started = true;
    ...
    if (speech_started && elapsed_seconds > 0.2) {
      auto result = _sence_voice->recog(buffer);
    ...
    if (data.size()) {
      ...
      // vad.Pop();
      auto result = _sence_voice->recog(data);
